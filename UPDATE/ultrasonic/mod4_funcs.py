...
def ultrasonic_read(Trigger, Echo):import RPi.GPIO as GPIO
import time
import smbus		    #import SMBus module of I2C
GPIO.setwarnings(False)     # Ignore warning for now
GPIO.setmode(GPIO.BOARD)    # Use physical pin numberin
...
def movingAvg(arr, position, numvals=3, wrap=1):
    # default to 3 pt moving average wi
    # trigger a reading
    GPIO.output(Trigger, True)
    time.sleep(0.00001)
    GPIO.output(Trigger, False)
...
def movingAvg(arr, position, numvals=3, wrap=1):
    ...
    for i in range(numvals):
        ...
        elif (position - i < 0 and wrap =import RPi.GPIO as GPIO
import time
import smbus		    #import SMBus module of I2C
GPIO.setwarnings(False)     # Ignore warning for now
GPIO.setmode(GPIO.BOARD)    # Use physical pin numberin
...
def MPU_Init(bus):
    ...
    SMPLRT_DIV     = 0x19
    CONFIG         = 0x1A
    GYRO_CONFIG    = 0x1Bimport RPi.GPIO as GPIO
import time
import smbus		    #import SMBus module of I2C
GPIO.setwarnings(False)     # Ignore warning for now
GPIO.setmode(GPIO.BOARD)    # Use physical pin numberin
...
def MPU_Read(bus, val2read):
    ...
    #some MPU6050 Registers and their Addimport RPi.GPIO as GPIO
import time
import smbus		    #import SMBus module of I2C
GPIO.setwarnings(False)     # Ignore warning for now
GPIO.setmode(GPIO.BOARD)    # Use physical pin numberin
...
def MPU_Read(bus, val2read):
    ...
    ACCEL_YOUT_H = 0x3D
    ACCEL_ZOUT_H = 0x3F
    GYRO_XOUT_H  = 0x43import RPi.GPIO as GPIO
import time
import smbus		    #import SMBus module of I2C
GPIO.setwarnings(False)     # Ignore warning for now
GPIO.setmode(GPIO.BOARD)    # Use physical pin numberin
...
def ultrasonic_read(Trigger, Echo):
    # trigger a readingimport RPi.GPIO as GPIO
import time
import smbus		    #import SMBus module of I2C
GPIO.setwarnings(False)     # Ignore warning for now
GPIO.setmode(GPIO.BOARD)    # Use physical pin numberin
...
def ultrasonic_read(Trigger, Echo):
    ...
    # Speed of sound 34300 cm/sec
    total_distance = (end_time - start_time) * 34300
    # Divide by 2, account for return tri
    GPIO.output(Trigger, True)
    time.sleep(0.00001)
    GPIO.output(Trigger, False)
    ...
    # Speed of sound 34300 cm/sec
    total_distance = (end_time - start_time) * 34300
    # Divide by 2, account for return triress
    Device_Address = 0x68   # MPU6050 device address
    ACCEL_XOUT_H = 0x3B
    ACCEL_YOUT_H = 0x3D
    ACCEL_ZOUT_H = 0x3F
...
def ultrasonic_read(Trigger, Echo):
    ...
    # Speed of sound 34300 cm/sec
    total_distance = (end_time - start_time) * 34300
    # Divide by 2, account for return tri= 1): 
            sumvals = sumvals + arr[(position - i)]
            count   = count + 1
        # wrap around to bottom of array with mod
        elif (position - i > array_size and wrap == 1):
...
def MPU_Init(bus):
    ...
    bus.write_byte_data(Device_Address, PWR_MGMT_1, 1)
    #Write to Configuration register
    bus.write_byte_data(Device_Address, C
    GYRO_YOUT_H  = 0x45
    GYRO_ZOUT_H  = 0x47
